...
export type IngestPdfFromAbsolutePathArgs = {
  ...
  /** 写入 manifest.embeddingModel（在线为方舟模型 ID，离线为 offline/stub） */
  manifestEmbeddingModel: string;
  /**
   * 为 true 时不校验 manifest 与当前模型 ID 一致（离线占位跨模型迁移时用）。
   */
...
export async function ingestPdfFromAbsolutePath(
  ...
  const embeddings = embeddingProviderToLangChain(embeddingProvider);
  const manifest = await readManifest(repoRoot);
  assertEmbeddingModelCompatible(manifest, manifestEmbeddingModel, {
    skipModelIdCheck: skipEmbeddingModelIdCheck === true,
  });
  ...
  const manifestOut = buildManifest(manifestEmbeddingModel, serializedRows, {
    source: sourceKey,

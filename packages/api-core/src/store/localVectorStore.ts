...
export type AssertEmbeddingModelCompatibleOptions = {
  /** 离线模式下允许 manifest 与占位模型 ID 不一致（仅靠向量维度对齐） */
  skipModelIdCheck?: boolean;
};
...
export function assertEmbeddingModelCompatible(
  ...
  currentModel: string,
  opts?: AssertEmbeddingModelCompatibleOptions,
): void {
  if (!manifest || opts?.skipModelIdCheck === true) {
    return;
  }
  if (manifest.embeddingModel && manifest.embeddingModel !== currentModel) {
    throw new Error(
      `kb_store 中记录的向量模型为「${manifest.embeddingModel}」，与当前嵌入模型「${currentModel}」不一致。请删除 kb_store 下的 ${VECTORS_FILENAME} / ${MANIFEST_FILENAME} 后重新 ingest，或改回同一模型。`,
    );
  }
}

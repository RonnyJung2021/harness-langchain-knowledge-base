...
import type { ChatMessage, CitationSummary, RagTurnInput, RagTurnOutput } from "@kb-rag/shared";
...
function baseMessageContentToString(content: BaseMessage["content"]): string {
...
export function baseMessagesToInference(messages: BaseMessage[]): InferenceChatMessage[] {
  return messages.map((m) => {
    const t = m._getType();
    const content = baseMessageContentToString(m.content);
    if (t === "system") {
...
export async function runRagChatTurn(
  ...
  const infMsgs = baseMessagesToInference(messages);
  const res = await deps.inference.chat(infMsgs);
  const assistantText = res.content.trim();
...
export async function runRagChatTurnStream(
  ...
  if (deps.inference.chatStream !== undefined) {
    const out = await deps.inference.chatStream(infMsgs, { onTokenDelta });
    return {
      assistantText: out.content.trim(),
      citations,
      degraded: degraded || undefined,
    };
  }

  const out = await deps.inference.chat(infMsgs);
  if (out.content.length > 0) {
    onTokenDelta(out.content);
  }
  return {
    assistantText: out.content.trim(),
    citations,

...
function requireNonEmptyEnv(name: string): string {
  ...
  if (raw === undefined || raw.trim() === "") {
    throw new Error(
      `缺少环境变量 ${name}：请复制 .env.example 为 .env 并填写，或在当前 shell 中导出该变量后再运行。（在线模式必填；离线模式可不设）`,
    );
  }
  return raw.trim();
}
...
/**
 * 从环境变量读取方舟 OpenAI 兼容接入配置。
 * **仅应由 Volcano Ark Provider 或显式在线 ingest 路径调用**。
 */
export function loadVolcanoArkEnvConfig(): ArkEnvConfig {
  ...
  const baseUrl = normalizeBaseUrl(
    baseRaw !== undefined && baseRaw.trim() !== "" ? baseRaw.trim() : DEFAULT_ARK_BASE_URL,
  );
  return {
    apiKey,
    baseUrl,
